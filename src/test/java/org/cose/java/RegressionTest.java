...
//    public /* NOT private */ String directoryName;
    ...
    public void ProcessDirectory(String directoryName) {
        ...
        assertNotNull(contents, "Directory " + directoryName + " should not be empty !");
        for (File f : contents) {
            ProcessFile(f.getAbsolutePath());
        }
        assertEquals(0, CFails, CFails + " tests failed in " + directoryName);
    }
    ...
    public void ProcessJSON(CBORObject control) throws Exception {
    ...
    public void _VerifyEncrypt(CBORObject control, byte[] rgbData) throws CoseException {
        ...
        try {
            ...
            try {
                ...
                if (Objects.equals(e.getMessage(), "Unsupported key size")) {
                    throw e;
                }
                if (!fFailBody && ((cnFail == null) || !cnFail.AsBoolean())) CFails++;
            } catch (Exception e) {
        ...
        for (iRecipient = 0; iRecipient < cnRecipients.size(); iRecipient++) {
            ...
            if (!cnRecipient.ContainsKey("recipients")) {
                ...
                for (iRecipient2 = 0; iRecipient2 < cnRecipient2.size(); iRecipient2++) {
                    if (!DecryptMessage(rgbEncoded, fFailBody, cnEnveloped, cnRecipient, iRecipient, cnRecipient2.get(iRecipient2), iRecipient2))
                        CFails++;
                }
            }
        }
    ...
    static CBORObject AlgorithmMap(CBORObject old) {
        ...
        switch (old.AsString()) {
            case "A128GCM":
                return AlgorithmID.AES_GCM_128.AsCBOR();
            case "A192GCM":
                return AlgorithmID.AES_GCM_192.AsCBOR();
            case "A256GCM":
                return AlgorithmID.AES_GCM_256.AsCBOR();
            case "A128KW":
                return AlgorithmID.AES_KW_128.AsCBOR();
            case "A192KW":
                return AlgorithmID.AES_KW_192.AsCBOR();
            case "A256KW":
                return AlgorithmID.AES_KW_256.AsCBOR();
            // case "RSA-OAEP": return AlgorithmID.RSA_OAEP.AsCBOR();
            // case "RSA-OAEP-256": return AlgorithmID.RSA_OAEP_256.AsCBOR();
            case "HS256":
                return AlgorithmID.HMAC_SHA_256.AsCBOR();
            case "HS256/64":
                return AlgorithmID.HMAC_SHA_256_64.AsCBOR();
            case "HS384":
                return AlgorithmID.HMAC_SHA_384.AsCBOR();
            case "HS512":
                return AlgorithmID.HMAC_SHA_512.AsCBOR();
            case "ES256":
                return AlgorithmID.ECDSA_256.AsCBOR();
            case "ES384":
                return AlgorithmID.ECDSA_384.AsCBOR();
            case "ES512":
                return AlgorithmID.ECDSA_512.AsCBOR();
            // case "PS256": return AlgorithmID.RSA_PSS_256.AsCBOR();
            // case "PS512": return AlgorithmID.RSA_PSS_512.AsCBOR();
            case "direct":
                return AlgorithmID.Direct.AsCBOR();
            ...
            case "AES-MAC-128/64":
                return AlgorithmID.AES_CBC_MAC_128_64.AsCBOR();
            case "AES-MAC-256/64":
                return AlgorithmID.AES_CBC_MAC_256_64.AsCBOR();
            case "AES-MAC-128/128":
                return AlgorithmID.AES_CBC_MAC_128_128.AsCBOR();
            case "AES-MAC-256/128":
                return AlgorithmID.AES_CBC_MAC_256_128.AsCBOR();
            case "AES-CCM-16-128/64":
                return AlgorithmID.AES_CCM_16_64_128.AsCBOR();
            case "AES-CCM-16-128/128":
                return AlgorithmID.AES_CCM_16_128_128.AsCBOR();
            case "AES-CCM-16-256/64":
                return AlgorithmID.AES_CCM_16_64_256.AsCBOR();
            case "AES-CCM-16-256/128":
                return AlgorithmID.AES_CCM_16_128_256.AsCBOR();
            case "AES-CCM-64-128/64":
                return AlgorithmID.AES_CCM_64_64_128.AsCBOR();
            case "AES-CCM-64-128/128":
                return AlgorithmID.AES_CCM_64_128_128.AsCBOR();
            case "AES-CCM-64-256/64":
                return AlgorithmID.AES_CCM_64_64_256.AsCBOR();
            case "AES-CCM-64-256/128":
                return AlgorithmID.AES_CCM_64_128_256.AsCBOR();
            case "HKDF-HMAC-SHA-256":
                return AlgorithmID.HKDF_HMAC_SHA_256.AsCBOR();
            case "HKDF-HMAC-SHA-512":
                return AlgorithmID.HKDF_HMAC_SHA_512.AsCBOR();
            case "HKDF-AES-128":
                return AlgorithmID.HKDF_HMAC_AES_128.AsCBOR();
            case "HKDF-AES-256":
                return AlgorithmID.HKDF_HMAC_AES_256.AsCBOR();
            case "ECDH-ES":
                return AlgorithmID.ECDH_ES_HKDF_256.AsCBOR();
            case "ECDH-ES-512":
                return AlgorithmID.ECDH_ES_HKDF_512.AsCBOR();
            case "ECDH-SS":
                return AlgorithmID.ECDH_SS_HKDF_256.AsCBOR();
            case "ECDH-SS-256":
                return AlgorithmID.ECDH_SS_HKDF_256.AsCBOR();
            case "ECDH-SS-512":
                return AlgorithmID.ECDH_SS_HKDF_512.AsCBOR();
            case "ECDH-ES+A128KW":
                return AlgorithmID.ECDH_ES_HKDF_256_AES_KW_128.AsCBOR();
            case "ECDH-SS+A128KW":
                return AlgorithmID.ECDH_SS_HKDF_256_AES_KW_128.AsCBOR();
            case "ECDH-ES-A128KW":
                return AlgorithmID.ECDH_ES_HKDF_256_AES_KW_128.AsCBOR();
            case "ECDH-SS-A128KW":
                return AlgorithmID.ECDH_SS_HKDF_256_AES_KW_128.AsCBOR();
            case "ECDH-ES-A192KW":
                return AlgorithmID.ECDH_ES_HKDF_256_AES_KW_192.AsCBOR();
            case "ECDH-SS-A192KW":
                return AlgorithmID.ECDH_SS_HKDF_256_AES_KW_192.AsCBOR();
            case "ECDH-ES-A256KW":
                return AlgorithmID.ECDH_ES_HKDF_256_AES_KW_256.AsCBOR();
            case "ECDH-SS-A256KW":
                return AlgorithmID.ECDH_SS_HKDF_256_AES_KW_256.AsCBOR();
            case "EdDSA":
                return AlgorithmID.EDDSA.AsCBOR();
            case "RSA-PSS-256":
                return AlgorithmID.RSA_PSS_256.AsCBOR();
            case "RSA-PSS-384":
                return AlgorithmID.RSA_PSS_384.AsCBOR();
            case "RSA-PSS-512":
                return AlgorithmID.RSA_PSS_512.AsCBOR();
