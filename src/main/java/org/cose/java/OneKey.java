...
public class OneKey {
    ...
    public OneKey(PublicKey pubKey, PrivateKey privKey) throws CoseException {
        ...
        if (pubKey != null) {
            ...
            if (Arrays.equals(alg.get(0).value, ASN1.oid_ecPublicKey)) {
                ...
                else if (Arrays.equals(oid, ASN1.Oid_brainpoolp256r1))
                    keyMap.Add(KeyKeys.EC2_Curve.AsCBOR(), KeyKeys.EC2_BP256R1);
                else if (Arrays.equals(oid, ASN1.Oid_brainpoolp320r1))
                    keyMap.Add(KeyKeys.EC2_Curve.AsCBOR(), KeyKeys.EC2_BP320R1);
                else if (Arrays.equals(oid, ASN1.Oid_brainpoolp384r1))
                    keyMap.Add(KeyKeys.EC2_Curve.AsCBOR(), KeyKeys.EC2_BP384R1);
                else if (Arrays.equals(oid, ASN1.Oid_brainpoolp512r1))
                    keyMap.Add(KeyKeys.EC2_Curve.AsCBOR(), KeyKeys.EC2_BP512R1);
                else throw new CoseException("Unsupported curve");

                byte[] keyData = spki.get(1).value;
    ...
    private void CheckECKey() throws CoseException {
        ...
        } else if (cn == KeyKeys.EC2_BP256R1) {
            oid = ASN1.Oid_brainpoolp256r1;
        } else if (cn == KeyKeys.EC2_BP320R1) {
            oid = ASN1.Oid_brainpoolp320r1;
        } else if (cn == KeyKeys.EC2_BP384R1) {
            oid = ASN1.Oid_brainpoolp384r1;
        } else if (cn == KeyKeys.EC2_BP512R1) {
            oid = ASN1.Oid_brainpoolp512r1;
        } else {
    ...
    static private OneKey generateECDSAKey(String curveName, CBORObject curve) throws CoseException {
        try {
            ...
            int curveSize = switch (curveName) {
                ...
                default -> throw new CoseException("Internal Error");
            };
...
//        try {
            ...
            byte[] rgbX = ((Ed25519PublicKeyParameters) keyPair.getPublic()).getEncoded();
            byte[] rgbD = ((Ed25519PrivateKeyParameters) keyPair.getPrivate()).getEncoded();
